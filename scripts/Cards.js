  ...
  var url = _getp('_url');
   ...
   var buycard = _BuyCard(url,_XML);
    for (var i = 1; i < buycard.length; i++) {
      ...
      if(cardinfo[0] <= _getp('Cards rarities to recycle')){
...
function _BuyCardAndRecycle(){//Buy and recycle
  ...
  var mathz = (Money - _getp('Auto buy limit')) / 1000;//Get the amount of cards you can buy,
  if(mathz <= 0){return false}
  var url = _getp('_url');
...
function _RecycleCard(URL,Card) {//Recycle card by Index
  var Salvage = UrlFetchApp.fetch(URL + '&message=salvageUnitList&units=%5b' + Card + '%5d');
  var Salvage_Json = JSON.parse(Salvage);
  return Salvage_Json.rewards.sp
}

function _UpgradeCard(URL,Card) {//Upgrade card by Index
  var Upgrade = UrlFetchApp.fetch(URL + '&message=upgradeUnit&unit_index=' + Card);
  var Upgrade_Json = JSON.parse(Upgrade);
  return Upgrade_Json.user_units[Card].level
}

function _BuyCard(URL,XML) {// 1 = Rarity, 2 = Index, 3 = item id
  var BuyPack = UrlFetchApp.fetch(URL + '&message=buyStoreItem&data_usage=0&expected_cost=1000&cost_type=2&item_id=1');
  var BuyPack_JSON = JSON.parse(BuyPack);
  if (BuyPack_JSON.result_message != null) {return false}
  var cards = ''
