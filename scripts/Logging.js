...
function WriteLogs(Section,Row) {
  ...
  if(string != null){sheet.getRange(Row+""+empty).setNote(string);}
...
function RewardsParse(rewards) {
  ...
  if(rewards_Json.gold != null){var Gold = rewards_Json.gold}else{var Gold = 0}
  if(rewards_Json.sp != null){var Sp = rewards_Json.sp}else{var Sp = 0}
  if(rewards_Json.xp != null){var xp = rewards_Json.xp}else{var xp = 0}
  if(rewards_Json.rating_change != null){var RatingChange = rewards_Json.rating_change}else{var RatingChange = 0}
  var items='', string=''
  if(rewards_Json.item != null){var itemlist = rewards_Json.item}
  else if(rewards_Json.items != null){var itemlist = rewards_Json.items}
  if(itemlist != null){
  for (var i = 0; i < itemlist.length; i++) {
    var item = itemlist[i].id;
    var Number = itemlist[i].number;
    if(item == 30001 || item == 30002){var item_name = 'Adcrate'}else{//They are removed from the inventory before it can id them.
      var item_name = ItemInfo_json.user_items[item].name;}
    items = items + ' ['+item_name+':'+Number+']'
  }
  }
  if(Gold != 0){string = string +' Gold:'+Gold}
  if(Sp != 0){string = string +' Wats:'+Sp}
